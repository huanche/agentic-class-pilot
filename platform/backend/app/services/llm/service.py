...
class LLMService:
    ...
    def __init__(self, model_name: str | None = None):
        ...
        Args:
            model_name: Registry entry to use as this service's default.
                ``None`` falls back to ``settings.DEFAULT_LLM_MODEL``. Each
                agent instantiates its own service so tool bindings and the
                fallback cursor stay isolated (inventory gap #5); the module
                singleton below remains ``ask.py``'s private instance.
        """
        ...
        try:
            self._current_model_index = all_names.index(target)
            self._llm = LLMRegistry.get(target)
            logger.info(
