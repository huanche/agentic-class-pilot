...
def test_reset_token_is_single_use(
    ...
    """A reset token carries a fingerprint of the password hash at issuance;
    after a successful reset, the same token must no longer work."""
    ...
    # reusing the same token must fail now that the password changed
    r = client.post(
        f"{settings.API_V1_STR}/reset-password/",
...
def test_reset_password(client: TestClient, db: Session) -> None:
    ...
    assert r.status_code == 200
    assert r.json() == {"message": "密码已重置，请使用新密码登录"}
